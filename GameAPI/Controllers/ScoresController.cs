...
namespace GameAPI.Controllers
{
    ...
    public class ScoresController : ControllerBase
    {
        ...
        /// </summary>
        [HttpGet("{id}")]
        public async Task<ActionResult<object>> GetPlayerScore(int id)
        {
        ...
        /// </summary>
        [HttpGet("top/{n}")]
        public async Task<ActionResult<IEnumerable<object>>> GetTopScores(int n = 10)
        {
            if (n < 1) n = 10;
            if (n > 100) n = 100;
            ...
            // Add rank
            var rankedScores = topScores.Select((s, index) => new
            {
                Rank = index + 1,
                s.Username,
                s.Score,
        ...
        /// </summary>
        [HttpPost]
        public async Task<ActionResult<object>> CreatePlayerScore(CreatePlayerScoreDto dto)
        {
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetPlayerHistory(string username)
        {
            var scores = await _context.PlayerScores
                .Where(s => s.Username.ToLower() == username.ToLower())
                .OrderByDescending(s => s.Score)
        ...
        public async Task<ActionResult<object>> GetStatistics()
        {
            ...
            var maxWave = await _context.PlayerScores.MaxAsync(s => (int?)s.Wave) ?? 0;
